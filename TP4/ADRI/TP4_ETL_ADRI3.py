...
fields=["ProductName","EngineVersion","AppVersion","AvSigVersion",
    "AVProductStatesIdentifier","AVProductsInstalled","AVProductsEnabled","HasTpm","CountryIdentifier",
...
#df=pd.read_csv("train_small.csv",usecols=fields,encoding="utf-8",dtype=dtypes)
df=pd.read_csv("C:\\TEMP\\TP4\\train.csv",nrows=200000,usecols=fields,encoding="utf-8",dtype=dtypes)
#df=pd.read_csv("train_cut.csv",encoding="utf-8",nrows=5000,dtype=dtypes)
#df=pd.read_csv("train.csv",nrows=10000,encoding="utf-8",dtype=dtypes)
...
########################################
#DUMMIFICACIONES/TRANSFORMACIONES A MANO
########################################
...
#Esto sirve
df_avsigversion_parts=df["AvSigVersion"].str.split(".",expand=True)
df["AvSigVersion"]=df_avsigversion_parts[0] + "." + df_avsigversion_parts[1] + df_avsigversion_parts[2] + df_avsigversion_parts[3]
del df_avsigversion_parts

df_osversion_parts=df["OsVer"].str.split(".",expand=True)
df["OsVer"]=df_osversion_parts[0] + "." + df_osversion_parts[1] + df_osversion_parts[2] + df_osversion_parts[3]
del df_osversion_parts

df_EngineVersion_parts=df["EngineVersion"].str.split(".",expand=True)
df["EngineVersion"]=df_EngineVersion_parts[0] + "." + df_EngineVersion_parts[1] + df_EngineVersion_parts[2] + df_EngineVersion_parts[3]
del df_EngineVersion_parts

df_appversion_parts=df["AppVersion"].str.split(".",expand=True)
df["AppVersion"]=df_appversion_parts[0] + "." + df_appversion_parts[1] + df_appversion_parts[2] + df_appversion_parts[3]
del df_appversion_parts

#dropCols=["OsBuildLab", "Census_InternalBatteryNumberofCharges","Census_OsVersion"]
